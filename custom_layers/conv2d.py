...
class Conv2D(nn.Module):
    def __init__(
        ...
        """Conv2D layer implemented from scratch.
    ...
    def init_weights(
        ...
        Args:
            ...
            uniform (bool): True to apply uniform distribution, False to normal distribution.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Tuple containing intitialized weights and biases:
                - torch.Tensor: Initialized weights tensor with previous shape.
                - torch.Tensor: Initialized bias tensor with previous shape.
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.amp.autocast(device_type=device.type, dtype=dtype):
            ...
            # [batch_size, in_channels, height_out, width_out, kernel_height, kernel_width]
            x_unf = x.unfold(2, kernel_height, S_H).unfold(3, kernel_width, S_W)
            # [batch_size, in_channels, output, kernel_height * kernel_width]
            x_unf = x_unf.contiguous().view(batch_size, in_channels, -1, kernel_height * kernel_width)
            # [batch_size * output, in_channels * kernel_height * kernel_width]
            x_unf = x_unf.permute(0, 2, 1, 3).reshape(batch_size * x_unf.shape[2], in_channels * kernel_height * kernel_width)
            ...
            if self.bias is not None:
                out += self.bias[None, :, None, None] # Add singleton dimensions for correct broadcasting
